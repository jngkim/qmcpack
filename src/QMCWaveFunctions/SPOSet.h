...
public:
  ...
  using OffloadVGLVector  = VectorSoaContainer<ValueType, DIM+2, OffloadPinnedAllocator<ValueType>>;
  ...
  */
  ...
  virtual void evaluateVGL(const ParticleSet& P, int iat, OffloadVGLVector& psi_vgl) {
    std::cout << "BADBAD evaluateVGL(psi_vgl) " << std::endl;
  }
