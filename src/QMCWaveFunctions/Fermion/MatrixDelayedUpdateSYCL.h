...
private:
  ...
  {
    if(m_queue==nullptr) m_queue = getSYCLDefaultDeviceDefaultQueue();
    return m_queue;
  }
  ...
  static void mw_prepareInvRow(const RefVectorWithLeader<This_t>& engines, const int rowchanged)
  {
    ...
    cudaErrorCheck(cuBLAS_MFs::gemv_batched(hstream, 'N', norb, delay_count, cone_vec.device_data(), V_mw_ptr, norb,
                   ...
                   "cuBLAS_MFs::gemv_batched failed!");
#else
    syclBLAS::copy_batched(lead_q, norb, (const Value**) oldRow_mw_ptr, 1, invRow_mw_ptr, 1, nw).wait();

    constexpr Value cone(1);
    constexpr Value cminusone(-1);
    constexpr Value czero{};

    syclBLAS::gemv_batched(lead_q, 'T', norb, delay_count, &cone, (const Value**)U_mw_ptr, norb,
                           (const Value**)invRow_mw_ptr, 1, &czero, p_mw_ptr, 1, nw).wait();
    syclBLAS::gemv_batched(lead_q, 'N', delay_count, delay_count, &cminusone, (const Value**)Binv_mw_ptr, lda_Binv, 
                           (const Value**)p_mw_ptr, 1, &czero, BinvRow_mw_ptr, 1, nw).wait();
    syclBLAS::gemv_batched(lead_q, 'N', norb, delay_count, &cone, (const Value**)V_mw_ptr, norb,
                           (const Value**)BinvRow_mw_ptr, 1, &cone, invRow_mw_ptr, 1, nw).wait();
#endif
    // mark row prepared
...
public:
...
#else
    SYCL::calcGradients(lead_q, norb, invRow_ptr, dpsiM_row_ptr, grads_value_v.device_data(), nw).wait();
    lead_q.memcpy(grads_value_v.data(), grads_value_v.device_data(), grads_value_v.size() * sizeof(Value)).wait();
#endif
                                  ...
                                  const std::vector<Value>& ratios)
  {
    ...
    constexpr Value cminusone(-1);
    constexpr Value czero{};
    syclBLAS::copy_batched(lead_q, norb, (const Value**)invRow_mw_ptr, 1, V_row_mw_ptr, 1, nw).wait();

    syclBLAS::gemv_batched(lead_q, 'T', norb, delay_count, &cminusone, (const Value**)V_mw_ptr,
                           norb, (const Value**)phiVGL_mw_ptr, 1, &czero, p_mw_ptr, 1, n_accepted).wait();

    syclBLAS::ger_batched(lead_q, delay_count, delay_count, cone_vec.device_data(), (const Value**)BinvRow_mw_ptr, 1,
                          (const Value**)BinvCol_mw_ptr, lda_Binv, Binv_mw_ptr, lda_Binv, n_accepted).wait();
...
*/
    {
      ...
      cublasErrorCheck(cuBLAS::gemm_batched(h_cublas, CUBLAS_OP_N, CUBLAS_OP_N, norb, norb, delay_count, &cminusone,
                       ...
                       "cuBLAS::gemm_batched failed!");
#else
      syclBLAS::gemm_batched(lead_q, 'T', 'N', delay_count, norb, norb, &cone,
                             (const Value**)U_mw_ptr, norb, (const Value**)Ainv_mw_ptr, lda, &czero, tempMat_mw_ptr, lda_Binv, nw).wait();
      SYCL::applyW_batched(lead_q, delay_list_mw_ptr, delay_count, tempMat_mw_ptr, lda_Binv, nw).wait();

      syclBLAS::gemm_batched(lead_q, 'N', 'N', norb, delay_count, delay_count, &cone,
                             (const Value**)V_mw_ptr, norb, (const Value**)Binv_mw_ptr, lda_Binv, &czero, U_mw_ptr, norb, nw).wait();

      syclBLAS::gemm_batched(lead_q, 'N', 'N', norb, norb, delay_count, &cminusone,
                             (const Value**)U_mw_ptr, norb, (const Value**)tempMat_mw_ptr, lda_Binv, &cone, Ainv_mw_ptr, lda, nw).wait();
#endif
    }
    delay_count = 0;
  }
