...
{
  constexpr size_t BS = 192;
  const size_t NB = (nrows+BS-1)/BS;
  return aq.parallel_for(sycl::nd_range<2>{{NB, NB * BS}, {1, BS}}, events, [=](sycl::nd_item<2> item) {
     const int col = item.get_global_id(1);
     if(col<nrows)
     {
       const int th = item.get_group(0);
       const int row_max = std::min((th+1)*BS,nrows);
       for(int row = th * BS; row < row_max; row++)
         mat[row*lda+col] = T{};
       if(item.get_group(0)  == item.get_group(1))
         mat[col*lda+col] = T(1);
     }
     });
}

template sycl::event make_identity_matrix_sycl(sycl::queue& aq,
                                               size_t nrows,
                                               double* restrict mat,
                                               size_t lda,
                                               const std::vector<sycl::event>& events);
...
template sycl::event make_identity_matrix_sycl(sycl::queue& aq,
                                               size_t nrows,
                                               std::complex<double>* restrict mat,
                                               size_t lda,
                                               const std::vector<sycl::event>& events);
} // namespace qmcplusplus
