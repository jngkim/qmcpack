                   ...
                   const std::vector<sycl::event>& events)
{
  ...
  const size_t blocks = std::min((array_size + tile_size-1) / tile_size, size_t(512));
  return aq.parallel_for(sycl::nd_range<1>{{tile_size*blocks},{tile_size}}, 
      events, [=](sycl::nd_item<1> item) {
      size_t id = item.get_global_id(0);
      size_t global_size = item.get_global_range(0);
      for (size_t i = id; i < array_size; i += global_size) {
          VC[i] = static_cast<T2>(VA[i]);
      }
      });
#endif
}
...
template sycl::event copy_n(sycl::queue& aq,
                            const std::complex<float>* restrict VA,
                            size_t array_size,
