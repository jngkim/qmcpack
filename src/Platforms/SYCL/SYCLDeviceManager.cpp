...
#pragma omp interop device(sycl_default_device_num) init(prefer_type("sycl"),targetsync: interop)
    ...
    for (int id = 0; id < devices.size(); id++)
      visible_devices.emplace_back(devices[id]); //context is automatically created with the device
...
{ // copy
  return sycl::queue{*default_device_queue}; 
}
