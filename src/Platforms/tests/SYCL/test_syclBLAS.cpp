...
void test_gemv(const int M_b, const int N_b, const char trans)
{
  ...
  //ompBLAS::gemv(m_queue, trans, N_b, M_b, alpha, B.device_data(), N_b, A.device_data(), 1, beta, C.device_data(), 1);

  syclBLAS::gemv(m_queue, trans, N_b, M_b, alpha, B.device_data(), N_b, A.device_data(), 1, beta, C.device_data(), 1)
      .wait();
